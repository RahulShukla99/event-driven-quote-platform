...
class QuotePricedListenerRetryAndDlqTest {
    ...
    void shouldRetryWorkflowAndPublishDlqAfterExhaustingAttempts() throws Exception {
        ...
        verify(deadLetterPublisher).publish(eq("quote.priced"), eq(payload), argThat(exception -> exception.getMessage().contains("db down")), eq(3));
    }
}
