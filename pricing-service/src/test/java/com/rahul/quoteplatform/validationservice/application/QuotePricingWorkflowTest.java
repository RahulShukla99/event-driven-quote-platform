...
class QuotePricingWorkflowTest {

    @Test
    void shouldSaveProcessedEventAndOutboxEventForValidatedQuote() {
        ...
        var processor = new QuotePricingProcessor(clock);
        var workflow = new QuotePricingWorkflow(processedEventRepository, outboxRepository, processor, clock, new ObjectMapper().findAndRegisterModules());
        var event = new QuoteValidatedEvent(
        ...
        assertThat(outboxRepository.events.getFirst().eventType()).isEqualTo("QuotePriced");
        assertThat(outboxRepository.events.getFirst().payloadJson()).contains("110.00");
    }
