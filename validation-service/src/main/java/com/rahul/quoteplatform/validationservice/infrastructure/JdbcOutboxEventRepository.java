...
import com.rahul.quoteplatform.validationservice.application.OutboxEventRecord;
import com.rahul.quoteplatform.validationservice.application.OutboxEventRepository;
...
public class JdbcOutboxEventRepository implements OutboxEventRepository {
    ...
    public void save(OutboxEventRecord event) {
        jdbcTemplate.update("""
                insert into validation_outbox_events (id, aggregate_id, aggregate_type, event_type, payload_json, created_at, published_at)
                values (?, ?, ?, ?, ?, ?, ?)
                """,
                event.id(),
                event.aggregateId(),
    ...
    public void markPublished(List<UUID> eventIds, java.time.Instant publishedAt) {
        for (UUID eventId : eventIds) {
            jdbcTemplate.update("update validation_outbox_events set published_at = ? where id = ?", Timestamp.from(publishedAt), eventId);
        }
    }
}
